...
function shareableUrl(options, query_part_only, include_fragment) {
    ...
    if ('search_index' in options) {
        ...
        // fall back to elasticsearch if not defined (better supported in original codebase)
        backend = 'elasticsearch';
    }

    if (backend == 'elasticsearch') {
        source = elasticSearchQuery({"options" : options, "include_facets" : options.include_facets_in_url, "include_fields" : options.include_fields_in_url});
    }
    else if (backend == 'solr') {
        source = solrQuery({"options" : options, "include_facets" : options.include_facets_in_url, "include_fields" : options.include_fields_in_url});
    }
    else {
        // TODO: Throw exception here - unrecognized
    }
...
(function($){
    $.fn.facetview = function(options) {
        ...
        function doSearch() {
            ...
            var query_function=null;
            if (backend == 'elasticsearch') {
                queryobj = elasticSearchQuery({"options" : options,
                                               "include_facets" : options.include_facets_in_url,
                                               "include_fields" : options.include_fields_in_url});
                query_function = doElasticSearchQuery;
            }
            else if (backend == 'solr') {
                queryobj = solrQuery({"options" : options,
                                      "include_facets" : options.include_facets_in_url,
                                      "include_fields" : options.include_fields_in_url});
                query_function = doSolrQuery;
            }
            else {
                // TODO: Throw exception here - unrecognized
            }
